...
function diffusion_1d(; n=1_000, nsteps=5_000_000)
    ...
    for step in 1:nsteps
        ...
        #
        MPI.Irecv!(@view(f_prev[1]), comm, reqs[1]; source=left_rank)
        MPI.Irecv!(@view(f_prev[n+2]), comm, reqs[2]; source=right_rank)
        MPI.Isend(@view(f_prev[2]), comm, reqs[3]; dest=left_rank)
        MPI.Isend(@view(f_prev[n+1]), comm, reqs[4]; dest=right_rank)

        # stencil computation
        #
        # TODO: Restrict the loop below to "inner cells"
        #
        for i in 3:n # for each cell that doesn't depend on ghost cells
        ...
        # 
        diffusion_stencil!(f, f_prev, c, 2)
        diffusion_stencil!(f, f_prev, c, n + 1)
